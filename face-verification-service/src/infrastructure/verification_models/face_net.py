...
class FaceNet(FaceVerificationModel):
    ...
    def verify(self, encoded_image, user):
        label_image = self._model.embeddings(self._image_to_tensor(user.image.encoded_image, expected_shape=(160, 160)))
        verification_image = self._model.embeddings(self._image_to_tensor(encoded_image, expected_shape=(160, 160)))
